...
function sendNotify(title, options, event) {
    ...
    const notificationPromise = self.registration.showNotification(title || 'Hi：', Object.assign({
        body: '这是一个通知示例',
        icon: '//image.yihaodianimg.com/front-homepage/mobile/images/index2017/logo.png',
        tag: 'push'
    }, options));
...
function onClickNotify(event) {
    ...
    const url = "https://wchaowu.github.io/pwa_demo/";
    event.waitUntil(
...
function offlineResponse(request) {
    ...
    if (request.url.match(/\.(jpg|png|gif|svg|jpeg)(\?.*)?$/)) {
        return caches.match('https://st.360buyimg.com/m/images/index/load/05003-lv.png');
    } else {
